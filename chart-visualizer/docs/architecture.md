...
从 v1.6.0 起，安装包还提供 `fengsha-diagram` CLI。它以非交互方式复用同一套浏览器渲染、标签修复和导出模块，详细命令见 [CLI 图文使用说明](cli-guide.md)。
...
![CLI 调用架构](assets/cli-architecture.svg)

CLI 的参数解析和 JSON 输出位于 Node 命令进程，真正的 Mermaid 渲染、文字测量、SVG 兼容处理、PNG/PDF 转换运行在一个不可见且隔离的 Electron 浏览器窗口。这个窗口加载构建产物中的专用 `cli.html`，不加载用户工作区，也不连接 AI 服务。

这条接缝让 CLI 与桌面界面共享 `diagram-engine`、`diagram-artifact` 和导出模块：桌面端后续修复字体、换行或清晰度时，CLI 会通过相同实现得到修复，不需要维护第二套渲染器。每次调用使用独立临时浏览器配置和随机本机端口，因此可以在桌面应用已经打开时运行，也不会争用用户数据目录。
