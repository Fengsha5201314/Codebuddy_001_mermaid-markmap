...
7. 方向确认后，在对话与输入区之间的操作栏点击 **生成候选**。内容会流式出现，也可以点击 **停止**。
   - 可以在 AI 输入框中直接按 `Ctrl+V` 粘贴截图；出现缩略图即表示已加入本次对话。图片最多 6 个，合计不超过 5 MB。
8. 等待“生成结构—本地编译—引用校验”完成，检查摘要和变更列表。Mermaid 候选会同时显示在中央画布，可用 **返回当前图** 切回原图。
9. 点击 **应用到画布**；也可以继续发送调整意见、更新候选或放弃候选。若结构校验失败，点击 **修复候选** 会只修复当前计划，不会要求重新描述需求。应用前，当前图始终保持不变。
...
应用到画布前，软件会保存“AI 修改前”版本快照。Mermaid AI 结果通过渲染检查后才允许应用；可视化 AI 先生成结构化计划，再由本地编译器生成 draw.io XML，并检查节点、连线、父级引用和 XML 结构。已有画布的 AI 修改采用局部补丁，未涉及的样式和坐标保持不变。候选生成后如果当前源码或画布又发生变化，软件会阻止直接覆盖并要求更新候选。
