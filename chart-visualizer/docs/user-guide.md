...
6. 需求还不完整时点击 **继续沟通**。AI 会基于当前图和该项目的历史对话归纳目标、指出关键问题，不会改图。
7. 确认方案后点击 **生成预览**。AI 会判断是修改、修复、转换、解释还是新建；内容会流式出现，也可以点击 **停止**。
8. 等待结构或渲染校验完成，检查摘要、变更列表和源码预览。
9. 点击 **确认应用**，或者继续输入调整意见后重新生成。

对话记录按项目保存。可在“项目对话”中开始新对话或切换历史记录；同一项目的 Mermaid 和可视化画布共享这些对话。通过“添加图片 / 文件”可提交图片、文本、Markdown、CSV、JSON、XML、Mermaid 和常见代码文件。DeepSeek 不支持图片；CPA / 自定义模型需先在设置中为具体模型开启“图片识别”。音频和视频不会被接收。
