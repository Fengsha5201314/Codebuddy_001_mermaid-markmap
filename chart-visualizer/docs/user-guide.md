...
这份指南面向第一次使用的用户，不要求了解编程。最简单的工作方式是：从模板开始，先在“画布 + 源码”中把业务逻辑做对，需要精细排版时再创建可视化副本，最后导出交付。
...
```mermaid
flowchart LR
    A["原 Mermaid 文档"] -->|"点击转为可视化"| B["新建 draw.io 可视化副本"]
    A --> C["原文档继续保留"]
    B --> D["自由拖拽、排版和样式修改"]
    D --> E["独立保存为画布 XML"]
    B -->|"返回 Mermaid 原图"| A
...
可视化副本和 Mermaid 原图是两份独立文档。转换后在画布中自由移动图形，不会反向覆盖原 Mermaid 源码。这样可以避免复杂排版被错误还原，也可以随时回到结构清晰的原图继续修改。
...
1. 打开右上角 **AI 助手**。
2. 从“本次使用”选择已启用模型。
3. 直接输入要求，或者点击 **优化结构、补全流程、排查问题、转换图种、解释与评审、从描述创建** 等任务模板，将专业提示词加入输入框。
4. 在模板内容后补充具体要求，例如“保留现有泳道，在付款前增加财务复核，驳回后返回申请人补充资料”。
5. 确认输入框上方显示“已识别当前 Mermaid 源码”或“已识别当前可视化画布”；当前页面无内容时会明确提示将创建新图。
6. 点击 **分析并生成预览**。AI 会根据当前图和需求判断是修改、修复、转换、解释还是新建；内容会流式出现，也可以点击 **停止**。
7. 等待结构或渲染校验完成，检查摘要、变更列表和源码预览。
8. 点击 **确认应用**，或者选择 **忽略**。

确认应用前，软件会保存“AI 修改前”版本快照。Mermaid AI 结果通过渲染检查后才允许应用；可视化 AI 结果会检查画布 XML 结构。

### AI 隐私提示

软件只把当前图表和本次指令交给你选择的服务，不会主动读取其他本地图表或旧版本。但是当前图表内容会经过对应模型提供商处理，因此不要发送密码、密钥、个人敏感数据或未经授权的内部资料。
...
通过 **更多 → 备份**，或者 **设置 → 数据与安全 → 下载完整备份**，可下载一个 JSON 文件，其中包含全部图表及版本。
...
### 可视化画布修改后，原 Mermaid 没有变化

这是当前设计：转换会创建独立副本。点击顶部 **返回 Mermaid 原图** 可以切回原文档，但不会把自由排版反向重写为 Mermaid。
