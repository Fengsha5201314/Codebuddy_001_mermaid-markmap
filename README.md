...
- 把 Mermaid 图转换为独立的可视化副本，在内置 draw.io 画布中拖拽、连线、调整样式和排版。
...
Mermaid 转换为可视化画布后，两份文档分别保存。可视化副本的自由排版无法可靠还原为所有 Mermaid 语法，因此目前不会把后续画布改动强制覆盖回原源码；顶部的 **返回 Mermaid 原图** 用于切换回原文档。
...
- 使用 AI 时，当前图表内容和本次指令会发送给所选模型服务，请不要提交不应交给该服务处理的敏感信息。
...
- Mermaid 转可视化会创建副本；可视化改动不会自动反向改写原 Mermaid 源码。
