...
任务模板只负责把专业系统提示词加入输入框，不会切换隐藏的操作模式；可在 **设置 → AI 指令模板** 中新增、修改、删除或恢复内置办公模板。只要当前页面有图表，AI 就会先读取当前图和该项目的对话历史，再判断需要修改、修复、转换或解释；页面无内容时才创建新图。可先点击 **继续沟通** 澄清需求，确认后再点 **生成预览**。AI 不会自动覆盖当前图表，生成完成后请检查结果，再点击 **确认应用**。具体配置见 [AI 模型接入说明](chart-visualizer/docs/ai-setup.md)。
